...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "agent_runs" / "seo_opportunity_agent"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Review SEO Automation opportunity queue rows and suggest safe next actions.")
    ...
    parser.add_argument("--from-bigquery", action="store_true", help="Read opportunity queue from BigQuery through the capped runner.")
    ...
    parser.add_argument("--limit", type=int, default=100, help="Maximum opportunity rows.")
    parser.add_argument("--output-json", help="Output path for validated run JSON.")
    return parser.parse_args()


def read_bigquery_opportunities(config: BigQueryCostConfig, *, limit: int, client_slug: str | None) -> list[dict]:
    ...
    sql = f"""
SELECT *
FROM `{config.project_id}.{config.reporting_dataset}.seo_opportunity_queue`
{client_filter}
...
LIMIT {int(limit)}
"""
    _, rows = runner.run_query(sql, purpose="seo-opportunity-agent: read opportunity queue")
...
def main() -> int:
    ...
    if should_write_bigquery and not args.from_bigquery:
        raise SystemExit("--write-bigquery requires --from-bigquery for SEO Opportunity Agent")
    ...
    start_agent_run_lifecycle(
        ...
        agent_name="SEO Opportunity Agent",
        started_at=started_at,
        mode=mode,
        prompt_version="seo_opportunity_agent/v001",
        input_sources=["agency_reporting.seo_opportunity_queue" if args.from_bigquery else "seo_automation_local_client_memory"],
    ...
    try:
        ...
        if args.from_bigquery:
            opportunities = read_bigquery_opportunities(config, limit=args.limit, client_slug=args.client_slug)
        else:
        ...
        context_pack = build_context_pack(
            ...
            run_id=run_id,
            created_at=started_at,
            task_type="seo_opportunity_review",
            source_tables=["agency_reporting.seo_opportunity_queue"],
            client_slug=args.client_slug,
            sections={"metrics": output.get("metrics", {}), "opportunity_count": len(opportunities)},
        )
        ...
        if should_write_bigquery:
            ...
            loaded = log_agent_output(
                ...
                batch_id=run_id,
                purpose="seo-opportunity-agent: log validated output",
            )
