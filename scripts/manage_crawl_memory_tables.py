...
from agency_bigquery.schema import ensure_crawl_memory_tables, plan_crawl_memory_tables  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plan or ensure AgencyOS technical crawl memory tables.")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--plan", action="store_true", help="Inspect table status without creating/updating anything.")
    mode.add_argument("--ensure", action="store_true", help="Create/update the approved crawl memory tables.")
...
def main() -> int:
    ...
    plan = plan_crawl_memory_tables(client, config)
